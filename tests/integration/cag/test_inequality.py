...
def test_inequality_with_numerical(data, metadata, constraint):
...
def test_inequality_unequal_datetime_formats_strings():
    """Test that the inequality constraint works with unequal datetime formats."""
    # Setup
    data = pd.DataFrame({
    ...
    constraint = Inequality(low_column_name='A', high_column_name='B')

    # Run
    sample = run_copula(data, metadata, [constraint]).sample(10)
...
def test_inequality_unequal_datetime_formats_timezone_aware():
    """Test that the inequality constraint works with timezone-aware datetime objects."""
    # Setup
    data = pd.DataFrame({
...
def test_inequality_multiple_constraints():
...
def test_inequality_with_nan():
    """Test that Inequality constraint works with NaN values."""
    # Setup
...
def test_auto_assign_transformer():
    """Test that the Inequality constraint works with auto-assign transformer."""
    # Setup
    data, metadata = download_demo('single_table', 'fake_hotel_guests')
    inequality_cag = Inequality(
        low_column_name='checkin_date',
